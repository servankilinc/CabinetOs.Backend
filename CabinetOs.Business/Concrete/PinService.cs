...
namespace CabinetOs.Business.Concrete
{
    public class PinService : IPinService
    {
        ...
        public async Task<Result<SelectList>> SelectListAsync(Expression<Func<Pin, bool>>? where = default, CancellationToken cancellationToken = default)
        {
            var list = await _unitOfWork.Pins.GetAllAsync<object>(select: s => new { s.Id, s.Name }, where: where, cancellationToken: cancellationToken);
        ...
        {
            var result = await _unitOfWork.Pins.PaginationAsync<PinDetailDto>(configurationProvider: _mapper.ConfigurationProvider, paginationRequest: request, include: i => i.Include(x => x.IoChanel).Include(x => x.Device), cancellationToken: cancellationToken);
        ...
        {
            var result = await _unitOfWork.Pins.DatatableClientSideAsync<PinDetailDto>(configurationProvider: _mapper.ConfigurationProvider, datatableRequest: request, include: i => i.Include(x => x.IoChanel).Include(x => x.Device), cancellationToken: cancellationToken);
        ...
        {
            var result = await _unitOfWork.Pins.DatatableServerSideAsync<PinDetailDto>(configurationProvider: _mapper.ConfigurationProvider, datatableRequest: request, include: i => i.Include(x => x.IoChanel).Include(x => x.Device), cancellationToken: cancellationToken);
