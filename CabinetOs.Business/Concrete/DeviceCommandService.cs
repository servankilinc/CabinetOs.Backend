...
namespace CabinetOs.Business.Concrete
{
    public class DeviceCommandService : IDeviceCommandService
    {
        ...
        public async Task<Result<SelectList>> SelectListAsync(Expression<Func<DeviceCommand, bool>>? where = default, CancellationToken cancellationToken = default)
        {
            var list = await _unitOfWork.DeviceCommands.GetAllAsync<object>(select: s => new { s.Id, s.PayloadJson }, where: where, cancellationToken: cancellationToken);
            var selectList = new SelectList(list ?? new List<object>(), "Id", "PayloadJson");
