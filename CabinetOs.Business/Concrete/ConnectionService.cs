...
namespace CabinetOs.Business.Concrete
{
    public class ConnectionService : IConnectionService
    {
        ...
        public async Task<Result<SelectList>> SelectListAsync(Expression<Func<Connection, bool>>? where = default, CancellationToken cancellationToken = default)
        {
            var list = await _unitOfWork.Connections.GetAllAsync<object>(select: s => new { s.Id, s.Label }, where: where, cancellationToken: cancellationToken);
            var selectList = new SelectList(list ?? new List<object>(), "Id", "Label");
